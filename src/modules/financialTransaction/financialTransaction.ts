...
import type { AllTransaction, GetTransaction } from "./financialTransactionTypes";
...
export async function getAllTransaction(client:MonimeClient):Promise<GetAllTransactionReturn>{
...
export async function getTransaction(client:MonimeClient, transactionId:string):Promise<GetTransactionReturn>{
    const { accessToken, monimeSpaceId} = client._getConfig()
    if(transactionId.trim() === ""){
        return { error:new Error("transactionId must not be empty"), success:false}
    }
